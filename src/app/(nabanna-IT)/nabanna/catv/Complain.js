...
import { EditComplainBtn,AddComplainBtn,SearchComplainBtn } from "../../component/Button/ComplainBtn";
...
const Complain = ({ setData }) => {
  ...
  const setDomain = "Cable_TV";
  const getCATVComplainData = async () => {
    try {
  ...
  useEffect(() => {
    ...
    // Auto Page Refresh//
    const intervalId = setInterval(getCATVComplainData, 10000); // Fetch data every 2 seconds
    return () => clearInterval(intervalId); // Cleanup on unmount*/
  }, []);
  return (
    <>
      <div className="flex justify-content-evenly items-center h-[60px]">
         <AddComplainBtn onTVSuccess={getCATVComplainData}/>
         <SearchComplainBtn domain={setDomain}/>
      </div>
